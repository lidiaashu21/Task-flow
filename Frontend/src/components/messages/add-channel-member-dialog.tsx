...
import { addConversationMember } from "@/lib/messages/api";
...
export function AddChannelMemberDialog({
  ...
  const mutation = useMutation({
    mutationFn: (userId: string) => addConversationMember(fetcher, conversationId, userId),
    onSuccess: async () => {
    ...
    onError: (error) => toast.error(error instanceof ApiError ? error.message : "Couldn't add this member"),
  });
